...
// \TODO Put your Next Fit code in this #ifdef block
   ...
   struct _block *lastAllocated = *last;
   // set curr to the location (header) after the last allocation, if it exists
   if (lastAllocated)
   {
      curr = lastAllocated->next;
   }

   // Iterate through the free list starting from lastAllocated (if lastAllocated is null, starts from beginning of list)
   while (curr && !(curr->free && curr->size >= size))
   {
      *last = curr;
      curr  = curr->next;
   }
   // if space was found, curr is not null and the location of that space is stored by *last
   // so that next time we search for free space, curr will be set to lastAllocated->next instead of beginning of heap

   // if no space was found, curr is null and end of the list has been reached
   if (curr == NULL)
   {
      // if lastAllocated is not Null (null would mean no space is available), then we should wrap back to beginning of list and keep searching
      if (lastAllocated)
      {
         curr = heapList;
         while (curr && !(curr->free && curr->size >= size))
         {
            // if we are back where we started, break the loop
            if (curr == lastAllocated)
            {
               // since while loop has not been broken, no space was found, curr is null
               curr = NULL;
               break;
            }
            *last = curr;
...
void *malloc(size_t size) 
{
  ...
  // if findFreeBlock found a _block, increment num_requested and num_reuses (since we didn't need to grow heap) 
   if (next)
   {
      num_requested += size;
      num_reuses += 1;
      ...
      {
         // initialize new block located after size has been allocated
         struct _block *new_block =  (struct _block *)((char *)next + size);
         // update new_block metadata
         new_block->size = leftover - sizeof(struct _block);
         new_block->next = next->next;
         new_block->free = true;
         // update original block metadata
         next->size = size;
         next->next = new_block;
         next->free = false;
         num_splits++;
   ...
   if (next == NULL) 
   {
      ...
      // if growHeap was successful, increment num_blocks, max_heap, and num_requested (since there were no free blocks found)
      if (next)
      {
         num_blocks += 1;
         max_heap += size;
   ...
   // increment num of mallocs that have been called
   num_mallocs += 1;
...
void free(void *ptr) 
{
  ...
  // increment total frees being called
  num_frees++;
